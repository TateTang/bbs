					...
					<div id="home" style="background-color: white;">
						...
						<div align="left"
							...
							<p>
								...
								${user.username } </font><br /> 昵称：								${user.nickname }
								<br />性别：	${user.sex }
								<br />邮箱：	${user.email }
								<br />职业： ${user.profession }
								<br />现居地：${user.come_from }
								<br />用户权限：
								<c:if test="${user.role_id==0}">普通用户</c:if>
								<c:if test="${user.role_id==6}">普通管理员</c:if>
								<c:if test="${user.role_id==16}" >超级管理员</c:if>
								<br />论坛等级：${user.userGrade.id }
								<br />论坛头衔：${user.userGrade.honor }
								<br />可用积分：${user.integral }
								<br />我的帖子数：${user.topic_count }
								&nbsp;&nbsp;&nbsp;&nbsp;我的评论数：${user.comment_count }
								<br />注册时间：
								<fmt:formatDate value="${user.register_time}" type="date" pattern="yyyy-MM-dd HH:mm:ss"/>
						</div>
						<div align="left"
							style="float: left;background-color: white;width: 360px;min-height: 420px;max-height:450px">
							<p>
								&nbsp;&nbsp;&nbsp;&nbsp;
								<img alt="我的头像" 	style="width: 140px;height: 200px;"  onerror="this.src='../image/chen-Java.png'" 
