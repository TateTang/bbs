		...
		<div class="tbody">
			...
			<div>
				...
				<div class="pageNav" align="right">
						<a href="topic_GetAllTopic.do?indexPage=1">
						<button style="width: 80px;">首页</button> </a>
						<a href="topic_GetAllTopic.do?indexPage=${allpage.indexPage==1?1:allpage.indexPage-1}">
						<button style="width: 80px;">上一页</button> </a>
						当前 ${allpage.indexPage }页，总共${allpage.pageCount }页
						<a href="topic_GetAllTopic.do?indexPage=${allpage.indexPage==allpage.pageCount?allpage.pageCount:allpage.indexPage+1}">
						<button style="width: 80px;">下一页</button> </a>
						<a href="topic_GetAllTopic.do?indexPage=${allpage.pageCount }">
			...
			<div class="listTopicDiv">
				<table align="left" class="listTopicStyle">
					<c:forEach items="${allList }" var="allTopic">
							<tr>
								<td style="width:160px;" align="left">
									<a href="cate_goCate.action?id=${allTopic.topicType.typeCategory.id }" target="_top" style="font-size: 14px;">
									${allTopic.topicType.typeCategory.name} 
									</a>-<a href="type_goType.action?id=${allTopic.topics_type_id} " target="_top" style="font-size: 14px;">
									${allTopic.topicType.name } 
									</a>
								</td>
								<td align="left">
									<div style="width:440px; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;float: left;">
										<font style="font-size: 13px;color: #4C9ED9">
											<c:if test="${allTopic.nice_topic ==1}">
												[<font color="red">精品</font>]
											</c:if>
											[<c:if test="${allTopic.status ==0}">未结帖</c:if>
											<c:if test="${allTopic.status ==1}">已结帖</c:if>]</font>
										<a href="topic_goTopic.do?id=${allTopic.id } " target="_top" class="topicTitle">${allTopic.title } </a>
									</div>
								</td>

								<td align="center" width="140px;">
									<a class="topicTitle" href="user_GoUser.action?user.id=${allTopic.topics_user_id }" target="_top" style="font-size: 14px;">
									${allTopic.topicUser.nickname } 
									</a>
								</td>
								...
								<td align="center" width="40px" style="font-size: 13px;">
									${allTopic.comment_count}
								</td>
								<td style="font-size: 12px;" width="110px;" align="right">
									<fmt:formatDate value="${allTopic.topic_time }" type="date" pattern="yyyy-MM-dd HH:mm"/>
								</td>
							</tr>
						</c:forEach>
					</table>
